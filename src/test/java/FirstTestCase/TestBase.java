...
public class TestBase {
    ...
    public void openURL()
    {
        open("https://чудопицца46.рф/");
    }

    @Step("Перехожу в рандомный пункт меню для оформления тестового заказа")
    public void mathRandomHead() {
        List<SelenideElement> mathRandomHead = elements(By.xpath("//div[@class = \"cat-list\"]/ul/li/a[contains(text(), \"\") and not (contains(@href, \"/actions\"))and not (contains(@href, \"/menu/combo\"))]"));
